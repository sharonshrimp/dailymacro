...
export default function Home() {
  ...
  if (dbLoading) return <div className="min-h-screen bg-slate-50 flex items-center justify-center font-black text-indigo-400 italic animate-pulse">LOADING DASHBOARD...</div>;
  ...
  return (
    <main className="min-h-screen bg-[#F0F4F8] p-4 pb-24 font-sans text-slate-900">
      <div className="max-w-md mx-auto">
        ...
        {/* --- Header 與日期選擇 --- */}
        <header className="mb-6 pt-4 px-2 text-center sm:text-left">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-1">Precision Nutrition Tracker</p>
        </header>
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={filteredMyFoods.map(f => f.id)} strategy={verticalListSortingStrategy}>
              {filteredMyFoods.map(food => (
                <SortableFoodItem key={food.id} food={food} onSelect={setSelectedFood} onEdit={handleEdit} onDelete={(id: any) => { if(confirm("Delete?")) { const newF = myFoods.filter(f => f.id !== id); setMyFoods(newF); syncToCloud(history, newF); } }} />
              ))}
            </SortableContext>
          </DndContext>
        </section>
